...
const geofenceIcon = L.icon({
  iconUrl:
    "https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.7.1/images/marker-icon.png", // Use the default Leaflet marker icon
  ...
  shadowUrl:
    "https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.7.1/images/marker-shadow.png", // Default shadow for Leaflet markers
      ...
      <div className="row">
        <div className="head">
          ...
          <div style={{ display: "flex", gap: "4px" }}>
            <CButton
              color="danger" // This makes the button red
        ...
        <div className={showRoutes ? "col-12" : "col-12"}>
          <div className="individualMap">
            ...
            >
              ...
              <Draggable>
                <CCard
                  className="mb-4 parametersContainer"
                  style={{ zIndex: "999" }}
                >
              ...
              {showGeofences &&
                matchingGeofences.map((geofence) => {
                  ...
                  if (parsedArea) {
                    ...
                    // );
                    const matchingGeoStatus =
                      geoStatus?.data?.filter(
                        (status) =>
                          status.geofenceName === geofence.name &&
                      ...
                      >
                        <Popup>
                          <div>
                            <h3>Geofence Details</h3>
                            <p>
                              <strong>Name:</strong> {geofence.name}
                            </p>
                            <p>
                              <strong>Crossed:</strong>{" "}
                              {geofence.isCrossed ? "Yes" : "No"}
                            </p>
                            <p>
                              <strong>Radius:</strong> {parsedArea.radius}{" "}
                              meters
                            </p>
                          </div>
                        </Popup>
                ...
                >
                  {matchingGeofences.length > 0 ? (
                    matchingGeofences.map((geofence, index) => {
                      ...
                      const filteredEta =
                        etaAlert?.filter(
                          (etaAlert) => etaAlert.geofenceName === geofence.name
                        ) || [];
                      console.log("filteredEta", filteredEta);
                      const etaTime = Math.floor(
                        filteredEta?.[0]?.etaTime || 0
                      );
                      const formattedEta =
                        etaTime >= 60
                          ? `${Math.floor(etaTime / 60)} h ${etaTime % 60} min`
                          : `${etaTime} min`;
                          ...
                          >
                            <Box
                              sx={{
                                ...
                                backgroundImage:
                                  "repeating-linear-gradient(to bottom,white 0%, white 40%,black 40%,black 60%,white 60%,white 100%)",
