...
const Export = ({ filteredRows, columnVisibility, COLUMNS, pdfTitle, pdfFilename, excelFilename, orangeBtn, prevColor}) => {
  ...
  const ExportToPdf = (orientation) => {
    ...
    filteredRows.forEach((row, index) => {
      ...
      COLUMNS().forEach((column) => {
        if (columnVisibility[column.accessor]) {
          let value;
          if(column.accessor=="lastUpdate"){
            value = column.accessor.split(".").reduce((acc, part) => acc && acc[part], row);
  ...
  return (
    <div>
      <Button onClick={handleClick} variant="contained"  style={orangeBtn ? orangeBtn : prevColor ? prevColor : { backgroundColor: "#d32f2f", color: "white" }} aria-haspopup="true" aria-label="Export options">
        Export
      </Button>
